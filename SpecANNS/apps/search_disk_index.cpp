...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    diskann::cout << std::setw(16) << "n_hops" << std::setw(16) << "n_cache_hit" << std::endl; 
    diskann::cout << "==============================================================="
                     "========================================================================================="
                  << std::endl;
    ...
    for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++)
    {
        ...
        // std::vector<std::vector<int>> cnt(query_num);
        // std::vector<std::vector<float>> n_times(query_num);
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        auto mean_hops = diskann::get_mean_stats<float>(
            stats, query_num, [](const diskann::QueryStats &stats) { return stats.n_hops; });
        auto mean_cache_hit = diskann::get_mean_stats<float>(
        ...
        diskann::cout << std::setw(16) << mean_hops << std::setw(16) << mean_cache_hit << std::endl;
        // std::ofstream io_writer("lsearch = " + std::to_string(L) + " io_time.csv");
        // for(int i=0; i<query_num;++i) {
        //     int _len = cnt[i].size();
        //     _len >>= 1;
        //     for(int j=0; j<_len; ++j) {
        //         io_writer << cnt[i][2*j] << "," << cnt[i][2*j+1] << "," << n_times[i][2*j] << "," << n_times[i][2*j+1] << ", ,";
        //     }
        //     io_writer << std::endl;
        // }
        delete[] stats;
