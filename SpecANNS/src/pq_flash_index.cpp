...
template <typename T, typename LabelT> inline uint64_t PQFlashIndex<T, LabelT>::get_node_sector(uint64_t node_id)
{
    ...
    return 1 + (_nnodes_per_sector > 0 ? (this->starling[node_id] >> 8)
                                        : node_id * DIV_ROUND_UP(_max_node_len, defaults::SECTOR_LEN));
}
...
inline char *PQFlashIndex<T, LabelT>::offset_to_node(char *sector_buf, uint64_t node_id)
{
    ...
    return sector_buf + (_nnodes_per_sector == 0 ? 0 : (this->starling[node_id] & 0xFF) * _max_node_len);
}
                                                      ...
                                                      const char *pivots_filepath, const char *compressed_filepath)
{
    ...
    int k = _disk_index_file.length() - 1;
    while(k >= 0) {
        if(_disk_index_file[k] == '/') break;
        k--;
    }
    std::string index_table_starling = _disk_index_file.substr(0, k + 1) + "index_table_starling.bin";
    ...
    this->starling = std::make_unique<uint32_t[]>(npts_u64);
    std::ifstream starling_reader(index_table_starling, std::ios::binary);
    starling_reader.read((char *)this->starling.get(), sizeof(uint32_t) * npts_u64);
    starling_reader.close();
...
void PQFlashIndex<T, LabelT>::cached_beam_search(const T *query1, const uint64_t k_search, const uint64_t l_search,
                                                 ...
                                                 const bool use_reorder_data, QueryStats *stats, std::vector<int>* cnt_stats, std::vector<float>* time_stats)
{
    cached_beam_search(query1, k_search, l_search, indices, distances, beam_width, std::numeric_limits<uint32_t>::max(),
                       use_reorder_data, stats, cnt_stats, time_stats);
}
                                                 ...
                                                 const bool use_reorder_data, QueryStats *stats, std::vector<int>* cnt_stats, std::vector<float>* time_stats)
{
    cached_beam_search(query1, k_search, l_search, indices, distances, beam_width, use_filter, filter_label,
                       std::numeric_limits<uint32_t>::max(), use_reorder_data, stats, cnt_stats, time_stats);
}
...
void PQFlashIndex<T, LabelT>::cached_beam_search(const T *query1, const uint64_t k_search, const uint64_t l_search,
                                                 ...
                                                 QueryStats *stats, std::vector<int>* cnt_stats, std::vector<float>* time_stats)
{
                                                 ...
                                                 QueryStats *stats, std::vector<int>* cnt_stats, std::vector<float>* time_stats)
{
    ...
    Timer query_timer, io_timer, cpu_timer, sum_timer;
    ...
    std::unordered_map<uint32_t, char *> starling_table_map;
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        while (retset.has_unexpanded_node() && frontier.size() < beam_width && num_seen < beam_width)
        {
            ...
            if (iter != _nhood_cache.end())
            {
                ...
                auto starling_iter = starling_table_map.find(get_node_sector(nbr.id));
                if(starling_iter != starling_table_map.end()) {
                    uint32_t *starling_node_buf = offset_to_node_nhood(offset_to_node(starling_iter->second, nbr.id));   // 计算该id的node的邻居的位置
                    uint32_t nnbrs = *starling_node_buf;
                    cached_nhoods.push_back(std::make_pair(nbr.id, std::make_pair(nnbrs, starling_node_buf + 1)));
                    if (stats != nullptr)
                    {
                        stats->n_cache_hits++;
                    }
        ...
        if(cnt_stats) {
            cnt_stats->push_back(num_seen);
            cnt_stats->push_back(frontier.size());
        }
        ...
        if (!frontier.empty())
        {
            ...
            if(time_stats) time_stats->push_back((float)io_timer.elapsed());
        }
        else if(time_stats) time_stats->push_back(0);
        ...
        for (auto &cached_nhood : cached_nhoods)
        {
            ...
            if(global_cache_iter != _coord_cache.end())
                ...
                node_fp_coords_copy = (T *)offset_to_node(starling_table_map[get_node_sector(cached_nhood.first)], cached_nhood.first);
            float cur_expanded_dist;
            if (!_use_disk_index_pq)
            {
        ...
        for (auto &frontier_nhood : frontier_nhoods)
        {
            ...
            if(starling_table_map.find(get_node_sector(frontier_nhood.first)) == starling_table_map.end()) {
                if(((uint64_t)frontier_nhood.second & 0xFFF)) 
                    diskann::cout<< num_sectors_per_node  << " : " << (uint32_t *)frontier_nhood.second << std::endl;
                char* starling_mem_ptr = new char[num_sectors_per_node * defaults::SECTOR_LEN];
                if(!starling_mem_ptr) diskann::cout << "alloc error !!!!" << std::endl;
                memcpy(starling_mem_ptr, frontier_nhood.second, num_sectors_per_node * defaults::SECTOR_LEN);
                starling_table_map[get_node_sector(frontier_nhood.first)] = starling_mem_ptr;
            }
        ...
        hops++;
        if(time_stats) time_stats->push_back((float)sum_timer.elapsed());
    }
    // re-sort by distance
...
#endif
    for(auto iter : starling_table_map) {
        delete[] iter.second;
    }
    if (stats != nullptr)
    {
