                        ...
                        const std::string reorder_data_file)
{
    ...
    { // Write multiple nodes per sector
        std::ofstream my_data_writer(output_file, std::ios::binary);
        uint32_t sector_num = 0;    // 已经分配的扇区数
        std::unique_ptr<uint32_t[]> starling = std::make_unique<uint32_t[]>(npts_64);   //每个节点存储到的扇区ID|扇区内位置
        std::unordered_map<uint32_t, uint32_t> sector_cnt;  //每个扇区存储的节点数
        std::unordered_map<uint32_t, char*> point_table;    // 每个扇区存储的内容，满了写入
        std::queue<char*> sector_pool;  // 扇区内存池，可以重复利用
        memset(starling.get(), -1, sizeof(uint32_t) * npts_64);
        for(uint32_t ids = 0; ids < npts_64; ++ids) {
            vamana_reader.read((char *)&nnbrs, sizeof(uint32_t));
            assert(nnbrs > 0);
            assert(nnbrs <= width_u32);
            // read node's nhood
            std::unordered_map<uint32_t, int> _cnt;
            ...
            int k;
            for(k = 0; k < nnbrs; ++k) {
                if(starling[nhood_buf[k]] == -1 || sector_cnt[starling[nhood_buf[k]] >> 8] >= nnodes_per_sector) continue;
                // if((starling[_nhood[k]] >> 8) >= n_sectors + 100) 
                //     diskann::cout << (starling[_nhood[k]] >> 8) << " is gteater than " << n_sectors + 100 << std::endl;
                _cnt[starling[nhood_buf[k]] >> 8]++;
            }
            k = -1;
            uint32_t __sector;
            char* __sector_ptr;
            ...
            if(k == -1) {
                ...
                if(!sector_pool.empty()) {
                    point_table[__sector] = __sector_ptr = sector_pool.front();
                    sector_pool.pop();
                    memset(__sector_ptr, -1, defaults::SECTOR_LEN);
                }
                else {
                    point_table[__sector] = __sector_ptr = new char[defaults::SECTOR_LEN];
                    memset(__sector_ptr, -1, defaults::SECTOR_LEN);
                }
            }
            else {
                __sector_ptr = point_table[__sector];
            }
            
            starling[ids] = (__sector << 8) | sector_cnt[__sector]; //重点
            __sector_ptr = __sector_ptr + sector_cnt[__sector] * max_node_len;
            // write coords of node first
            // 拷贝节点数据到扇区中
            memcpy(__sector_ptr, cur_node_coords.get(), ndims_64 * sizeof(T));
            // write nnbrs
            *(uint32_t *)(__sector_ptr + ndims_64 * sizeof(T)) = (std::min)(nnbrs, width_u32);
            memcpy(__sector_ptr + ndims_64 * sizeof(T) + sizeof(uint32_t), nhood_buf,
                       (std::min)(nnbrs, width_u32) * sizeof(uint32_t));
            if(++sector_cnt[__sector] == nnodes_per_sector) {
                // 写入扇区，并把指针移入内存池
                my_data_writer.seekp((__sector + 1) * defaults::SECTOR_LEN, std::ios::beg);
                my_data_writer.write(point_table[__sector], defaults::SECTOR_LEN);
                sector_pool.push(point_table[__sector]);
                point_table.erase(__sector);
            }
        }
        diskann::cout << "finish write " << sector_num - point_table.size() << "sectors, but " << point_table.size() << " sectors don't write" << std::endl;
        for(auto iter : point_table) {
            my_data_writer.seekp((iter.first + 1) * defaults::SECTOR_LEN, std::ios::beg);
            my_data_writer.write(iter.second, defaults::SECTOR_LEN);
            delete[] iter.second;
        }
        while(!sector_pool.empty()) {
            auto f = sector_pool.front();
            sector_pool.pop();
            delete[] f;
        }
        int k = base_file.length() - 1;
        while(k >= 0) {
            if(base_file[k] == '/') break;
            k--;
        }
        std::string index_table_starling = base_file.substr(0, k + 1) + "index_table_starling.bin";
        std::ofstream starling_writer(index_table_starling.c_str(), std::ios::binary);
        starling_writer.write((char *)starling.get(), sizeof(uint32_t) * npts_64);
        if(starling_writer.is_open()) starling_writer.close();
        my_data_writer.close();
